...
class Scanner {
  constructor (foundHostCallback, maxRetries = 5, maxSimultaneousRequests = 2) {
    this.scannedHosts = []
    this.scanQueue = []
  ...
  async walkHosts(start, regex, attempt = 0) {
    const thisHost = {};
    try {
      console.log(`Trying node: ${start}`);
      const nodes = await axios({
        method: 'get',
        url: `http://${start}.local.mesh:8080/cgi-bin/sysinfo.json?hosts=1&link_info=1&lqm=1`,
        timeout: parseInt(process.env.TIMEOUT || '60000') // 1m
      })
      ...
      if (attempt < this.maxRetries) {
        return this.walkHosts(start, regex, attempt + 1)
      }
    }
...
new Scanner(
  callback,
  parseInt(process.env.RETRIES || '5'),
